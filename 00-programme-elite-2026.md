...
<!-- subtitle: Préparation Superday S&T / dérivés / structurés — 22 jours, 6 h par jour, bilingue FR/EN -->
<!-- date: 5 → 30 septembre 2026 · 22 jours · version 1.1 (améliorée) -->
<!-- meta: Desk cible n°1 : Equity derivatives & produits structurés (n°2 rates/ALM/repo, n°3 commo pétrole + coton) -->
<!-- meta: Périmètre : 100 % du Drive (61/61 fichiers). Zéro notion hors Drive, sauf approfondir ou voir un concept sur le marché. -->
<!-- meta: Lab : ShockDesk (onglets réels) + calvin-exotic-desk — aucun onglet inventé. -->
<!-- meta: Cerveau du mois : github.com/Calvin29990/Masterclass-front · Lab : github.com/Calvin29990/shockdesk -->
<!-- meta: Règle d'or : un chiffre sans provenance n'est pas un chiffre. -->

## 1. L'essentiel en 1 page

Un **mois**, un seul objectif : arriver au Superday Front Office sans mourir en
S&T / structuration / junior trading dérivés. Un mois ne fait pas de toi un
trader élite — ça se joue sur un desk, pas dans un Drive. Il fait de toi un
*candidat junior élite*.

| | |
|---|---|
| **Desk visé (un seul)** | n°1 **Equity derivatives / produits structurés** (sales-trader, junior trader, structurer). n°2 Rates/ALM/repo. n°3 Commo pétrole + coton. |
| **Périmètre** | 100 % du Drive (61/61 fichiers, voir `02-catalogue-drive.md`). Zéro notion hors Drive. |
| **Stage** | Pause jusqu'au 30/09. Une candidature = faute `F`. |
| **Lab** | ShockDesk (onglets réels) + calvin-exotic-desk. Pas d'onglet ALM, pas d'onglet Risk, pas de galerie d'autocalls. |
| **Rythme** | 6 h 00 par jour, 22 jours. Compressé 4 h 30 (1 jour de panne max). Dimanche off = 20 min de flashcards. |
| **Livrable du jour** | `livrables/jXX_*.py` — absent ⇒ jour non clos, dimanche off annulé. |
| **3 trade ideas** | 11/09 Oil timing (ShockDesk) · 18/09 Structuré (Phoenix/collar) · 25/09 Rates 2s10s **ou** coton WASDE. |
| **Chaque vendredi** | Mini-Superday 45 min enregistrées (11, 18, 25, 30). |

Septembre n'a pas de 31. Fin = **mercredi 30**.

## 2. Le contrat de journée — 6 h, pas 4 h

Un Superday FO dure 4 à 6 h d'oraux. S'entraîner 4 h et se reposer jeudi *et*
dimanche, c'est se préparer à perdre.
...
| Bloc | Temps | Règle |
|---|---|---|
| 0. Mental math | 0 h 15 | Optiver Guide, chrono, **à voix haute**, dès J1 |
...
| F. Brainteasers | 0 h 30 | 3 problèmes ; raté → refait le lendemain *avant* les nouveaux |
...
1 fit + 1 trade idea 3 min. Tu te réécoutes. Non négociable.
...
## 3. Ce que « élite » veut dire le 30/09 au soir

Sans notes, en 8 minutes, dans les deux langues :
...
2. Donner le signe de Δ, Γ, ν, Θ sur un long straddle et *montrer* le choc
   +10 pts d'IV sur ShockDesk (Atelier 7, 10,69 → 23,91).
3. Choisir la structure pour : haussier, baissier, range, choc d'amplitude,
   timing de niveau — et dire pourquoi le strangle gagne *par dollar*, le
...
## 4. Chiffres ShockDesk à connaître par cœur

**Provenance :** yfinance · `shock-lab-oil` · 25,5 M$ · 2026-07-01 → 2026-08-29
· 42 barres. **Jamais les chiffres synthétiques** (le générateur inventait
« +395 919 $ / −4 054 $ » — faux sur Yahoo).

| | Sortie pic J+7 | Stop calendar J+21 |
|---|---|---|
| P&L | **+337 887 $ (+1,32 %)** | **−279 633 $ (−1,10 %)** |
| Valeur du timing | **617 520 $** | |
| Brent (BZ=F) | +187,1 k$ | −117,5 k$ |
| S&P 500 (short ^GSPC) | +82,0 k$ | −213,4 k$ |
| Or (GC=F) | **+53,0 k$** (pas un miss) | +107,2 k$ |
...
**Drive S :** WORDS *Questions de base* (forward, futures, repo, Sharpe) + *Questions Produits dérivés* (call/put, EU/US).

| Bloc | Consigne |
|---|---|
| Cours | Hypothèses BS (6). Parité call-put. Forward F = S·e^((r−q)T) ; FX F = S·e^((r−rf)T) ; commo F = S·e^((r+u−y)T). Forward vs futures (marge, MTM). |
...
**Ticket de sortie :** réciter les 6 hypothèses + écrire la parité les yeux fermés.
...
**Ticket de sortie :** strangle 10,69 → 23,91 (+10 pts d'IV) reproduit sur ShockDesk.
...
| Lab | ShockDesk : construire les 5. Noter débit/crédit (prime nette ≥ 0 = **débit**). Straddle vs strangle : prime ×2,3 (24,40 vs 10,69) ; à +18,4 % le straddle gagne *par structure*, le strangle *par dollar* (×2,13). |
...
**Ticket de sortie :** la matrice 2×2 direction × vol sans notes.

### Mer 09/09 — J4 · Smile, skew, surface

**Drive P :** Natenberg vol + Gatheral ch. 1–3 (sticky strike / sticky delta, term structure).
**Drive S :** — (pas de papier d'arbitrage aujourd'hui, c'est J10/J16).
...
| Lab | ShockDesk : `iv_shift` et `vol_regime`. Observer OTM, vega local, BE. Relier au filtre `MIN_EDGE` de `long-strangle-shock.py` (edge 0,52 < 1,00 → **0 trade**). Le marché price 18,4 %, le modèle r1 price 5 %. |
| Marché | Surface SPX ou EURUSD si accès ; sinon VIX term structure. |
...
**Ticket de sortie :** expliquer le skew 1987 + pourquoi le strangle ne trade pas (edge 0,52).
...
**Ticket de sortie :** 337 887 vs −279 633, et qui est le miss (HYG/TLT, pas l'or).
...
**Ticket de sortie :** 20 oraux (le mini-Superday) tenus et réécoutés.
...
**Ticket de sortie :** KO < vanille (et pourquoi).
...
**Ticket de sortie :** 8 définitions d'exotiques en 20 s chacune.
...
**Ticket de sortie :** payoff Phoenix en 3 régimes écrit noir sur blanc.
...
**Ticket de sortie :** le test de convexité (butterfly arb) tourne.
...
**Ticket de sortie :** RR + fly reconstruits à la main.
...
| Bloc | Consigne |
|---|---|
| Cours | Convertible ≈ bond + call (taux + crédit + vol). Asymétrie upside equity / downside bond. Pourquoi un arb desk est long vol / long crédit parfois. |
...
**Ticket de sortie :** bond + call (décomposition du convertible).
...
**Ticket de sortie :** YTM par Newton sur 3 itérations.
...
**Ticket de sortie :** même D → on achète la plus convexe.
...
**Ticket de sortie :** NPV 0 à l'origine (IRS) expliqué.

### Mer 23/09 — J16 · ALM, liquidité, xVA (vue desk)

**Drive P :** Corlosquet-Habart (ALM, chapitres banque : gap, IRRBB) + Banks *Liquidity Risk* (funding vs market liquidity) + Gregory *xVA* ch. 1–4 (CVA, DVA, FVA — vocabulaire).
...
**Ticket de sortie :** immunisation + CVA en une phrase chacun.
...
**Ticket de sortie :** special vs GC expliqué.
...
**Ticket de sortie :** contango ≠ backwardation, sans les inverser.
...
**Ticket de sortie :** 25 problèmes dans le journal (temps + correction).
...
**Drive S :** matrix cookbook (formules réellement utilisées) + WORDS *Entrainement questions* (proba risque-neutre, parité).
...
**Ticket de sortie :** MC vs PDE vs closed form.
...
**Ticket de sortie :** ES cohérente (sous-additive), VaR non.

### Mer 30/09 — J22 · Grand oral + pitch + GitHub

**Drive P :** *Why Quant.pdf* (motivation, 45 min max) + WORDS *FICHE-ENTRETIEN* + *Questions-SG* + *Aide Maxime* + *Questions générales stage* (fit **uniquement**, tu ne candidates pas).
...
**Ticket de sortie :** pitch 3 min tenu sans bégayer.

**Fin du sprint.** Octobre = recherche (fichier `03`). Pas de candidature avant que le pitch tienne 3 min.

---

## 6. Rotation brainteasers (rappel)
...
## 7. Ce que tu n'ouvres pas en septembre (volontaire)

Rejda · Sandström Solvency · Lopez de Prado · ML finance · Quant Investing
Python · Trading with R · Convertible Arbitrage au-delà des ch. 1–3 · Gregory
au-delà des ch. 1–4 · WASDE en série · Cotton au-delà d'une journée.

C'est dans `03-octobre-base-recherche.md`. Ce n'est pas un oubli, c'est une décision.

## 8. Provenance & chiffres d'or (à recoller si on te dit que tes refs sont fausses)
...
- Stage : pause. LinkedIn ouvert pour candidater → jour marqué `F` (faute).
- Livrable Python absent → jour non clos.
- Dimanche off annulé si un ticket de sortie de la semaine est rouge.

Bloc à recoller dans le chat (un jour) :
